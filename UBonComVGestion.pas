...
procedure TBonComVGestionF.F3Execute(Sender: TObject);
begin
   //--- this is to focus in produit --------------------------
  if  AddBVComBonComGBtn.Enabled = False then
  begin
     ProduitBonComGCbx.SetFocus;
  end;
end;

procedure TBonComVGestionF.F4Execute(Sender: TObject);
begin
  if AddBVComBonComGBtn.Enabled = True then
  begin
     AddBVComBonComGBtnClick(Screen);
  end;
end;

procedure TBonComVGestionF.F5Execute(Sender: TObject);
begin
  if EditBVComBonComGBtn.Enabled = True then
  begin
     EditBVComBonComGBtnClick(Screen);
  end;
end;

procedure TBonComVGestionF.F6Execute(Sender: TObject);
Var I :Integer;
begin
  //--- this is to switch between produits and quntity--------------------------
  if EditBVComBonComGBtn.Enabled = False then
  begin
...
procedure TBonComVGestionF.F7Execute(Sender: TObject);
Var I: Integer;
begin
...
procedure TBonComVGestionF.F8Execute(Sender: TObject);
begin
  if EditBVComBonComGBtn.Enabled = False then
  begin
...
procedure TBonComVGestionF.F9Execute(Sender: TObject);
begin
  if ValiderBVComBonComGBtn.Enabled = True then
  begin
    ValiderBVComBonComGBtnClick(Screen);
  end;
end;

procedure TBonComVGestionF.F10Execute(Sender: TObject);
begin
  if EditBVComBonComGBtn.Enabled = False then
  begin
     ListClientBonComGBtnClick(Screen);
  end;
end;

procedure TBonComVGestionF.F11Execute(Sender: TObject);
begin
...
procedure TBonComVGestionF.F12Execute(Sender: TObject);
var
Ini: TIniFile;
indexP: Integer;
begin
    ...
    if ModePaieBonComGCbx.Text = '' then
     ...
     //---------------------------------------------------
         ...
         begin
          ...
          DataModuleF.Bonv_comTable.FieldByName('code_c').AsInteger:= MainForm.ClientTable.FieldByName('code_c').AsInteger;
          DataModuleF.Bonv_comTable.FieldByName('date_bvcom').AsDateTime:= DateBonComGD.DateTime;
          ...
          DataModuleF.Bonv_comTable.FieldByName('obser_bvcom').AsWideString:= ObserBonComGMem.Text;
          DataModuleF.Bonv_comTable.FieldByName('num_cheque_bvcom').AsString:= NChequeBonComGCbx.Text;
          DataModuleF.Bonv_comTable.FieldByName('montht_bvcom').Value:= StrToFloat(StringReplace(BonComTotalHTLbl.Caption, #32, '', [rfReplaceAll]));
          DataModuleF.Bonv_comTable.FieldByName('montaht_bvcom').Value:= StrToFloat(StringReplace(BonComTotalAHTLbl.Caption, #32, '', [rfReplaceAll]));

          if RemiseBonComGEdt.Text<>'' then
          begin
             DataModuleF.Bonv_comTable.FieldByName('remise_bvcom').Value:=StrToFloat(StringReplace(RemiseBonComGEdt.Text, #32, '', [rfReplaceAll]));
          end else begin
                    DataModuleF.Bonv_comTable.FieldByName('remise_bvcom').Value:=0;
                   end;


          DataModuleF.Bonv_comTable.FieldByName('montver_bvcom').Value:=StrToFloat(StringReplace(BonComRegleLbl.Caption, #32, '', [rfReplaceAll]));
          DataModuleF.Bonv_comTable.FieldByName('montttc_bvcom').Value:=StrToFloat(StringReplace(BonComTotalTTCLbl.Caption, #32, '', [rfReplaceAll]));
...
//        MainForm.SQLQuery.DisableControls;
        MainForm.SQLQuery.Active:=False;
        MainForm.SQLQuery.SQL.Clear;
        MainForm.SQLQuery.SQL.Text:= 'SELECT code_p,prixht_p,prixvd_p,prixvr_p,prixvg_p,prixva_p,prixva2_p,tva_p,perissable_p FROM produit WHERE LOWER(nom_p) LIKE LOWER('+QuotedStr(ProduitBonComGCbx.Text)+')';
        MainForm.SQLQuery.Active:=True;
...
//--------------------------------------------------------------------------------------------------------------------
  ...
  begin
            MainForm.SQLQuery.Active:=False;
            MainForm.SQLQuery.SQL.Clear;
            MainForm.SQLQuery.SQL.Text:= 'SELECT code_p,refer_p,prixht_p,prixvd_p,prixvr_p,prixvg_p,prixva_p,prixva2_p,tva_p,perissable_p FROM produit WHERE LOWER(refer_p) LIKE LOWER('+QuotedStr(ProduitBonComGCbx.Text)+')';
            MainForm.SQLQuery.Active:=True;
            CodeP:= MainForm.SQLQuery.FieldByName('code_p').AsInteger ;
 ...
 //---------------------------------------------------------------------------------------------
  ...
  begin
   ...
   begin
    ...
    MainForm.SQLQuery.Active:=False;
    MainForm.SQLQuery.SQL.Clear;
    MainForm.SQLQuery.SQL.Text:= 'SELECT code_p,codebar_p,prixht_p,prixvd_p,prixvr_p,prixvg_p,prixva_p,prixva2_p,tva_p,perissable_p FROM produit WHERE code_p = '+QuotedStr(IntToStr(CodeCB)) +'OR'+ ' LOWER(codebar_p) LIKE LOWER(' + QuotedStr(ProduitBonComGCbx.Text)+')';
    MainForm.SQLQuery.Active:=True;
    CodeP:= MainForm.SQLQuery.FieldByName('code_p').AsInteger ;
...
//  end;
//
//  if (GetKeyState(VK_F9) < 0) AND (ValiderBVComBonComGBtn.Enabled = True)
...
begin
 if DataModuleF.Bonv_comTable.FieldByName('valider_bvcom').AsBoolean = True then
  ...
  begin
  BonComVGestionF.ModePaieBonComGCbx.Text:=DataModuleF.Bonv_comTable.FieldByName('ModePaie').AsString;
  end;
  if (DataModuleF.Bonv_comTable.FieldByName('code_cmpt').AsInteger<> 0) AND (DataModuleF.Bonv_comTable.FieldByName('code_cmpt').AsInteger<>null)  then
  begin
  BonComVGestionF.CompteBonComGCbx.Text:=DataModuleF.Bonv_comTable.FieldByName('Compte').AsString;
  end;

  if  (DataModuleF.Bonv_comTable.FieldByName('MontantRes').AsFloat<>null)  then
  begin
  BonComVGestionF.BonComResteLbl.Caption:=CurrToStrF(((DataModuleF.Bonv_comTable.FieldByName('MontantRes').AsFloat)),ffNumber,2) ;
  end;

   BonComVGestionF.NumBonComGEdt.Caption:= DataModuleF.Bonv_comTable.FieldByName('num_bvcom').AsString;
   BonComVGestionF.DateBonComGD.DateTime:= DataModuleF.Bonv_comTable.FieldByName('date_bvcom').AsDateTime;
   BonComVGestionF.ObserBonComGMem.Text:= DataModuleF.Bonv_comTable.FieldByName('obser_bvcom').AsString;
...
begin
   ...
   BonComVGestionF.BonComGClientOLDCredit.Caption:= FloatToStrF(((DataModuleF.SQLQuery3.FieldByName('credit_c').AsFloat)),ffNumber,14,2);
   BonComVGestionF.BonComGClientNEWCredit.Caption:=FloatToStrF(0,ffNumber,14,2);
...
begin
  if (ClientBonComGCbx.Text <> '')
    and (DataModuleF.Bonv_comTable.FieldByName('code_c').AsInteger <> 0)
    and (DataModuleF.Bonv_comTable.FieldValues['code_c'] <> Null )  then
  ...
  //  DataModuleF.Bonv_comTable.Refresh;
    ...
    FullfillFormBonComV();
     ...
     FullfillCredits(DataModuleF.Bonv_comTable.FieldByName('code_c').AsInteger);
  end else
    begin
      sndPlaySound('C:\Windows\Media\Windows Hardware Fail.wav', SND_NODEFAULT Or SND_ASYNC Or SND_RING);
      ClientBonComGCbx.StyleElements:= [];
